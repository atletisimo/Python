...
if __name__=='__main__':
    ...
    #treniranje na klasifikator
    for item in train_data:
        klasifikator.train(item[0],item[1])


    sorted_words=sorted(get_words(recenica))
    print(sorted_words)

#word e sekoj zbor od sortiranata lista treba da go pecatime zborot,klasata i soodvetno da pecatime verojatnosta konkretniot zbor da spagja vo nekoja klasa
    for word in sorted_words:
        #primer zborot spagja vo kategorija sport so 0.1 itn
        #da gi zememe site kategorii
        all_categories=klasifikator.categories()#site kategorii koi se koristele pri gradenje na klasifikatorot
        #print(all_categories)
        for current_class in all_categories:
            print("Zborot:{},Klasa{},2te verojatnosti {},{}".format(word,current_class,
                                                                    klasifikator.get_feature_per_category_probability((word,current_class),5)
                                                                      round( klasifikator.weighted_probability(word,current_class,klasifikator.get_feature_per_category_probability)
