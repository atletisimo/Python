...
# За тренинг да се земат само првите 40 примероци од класите Roach и Pike
# (Првите 40 од Roach и првите 40 од Pike). Значи треба да се направи бинарен класификатор,
# при што секоја класа освен _Roach_ и _Pike_ се игнорира при тренирањето на множеството.
...
from Drva import build_tree,classify,get_max_predition
...
if __name__ == "__main__":
    ...
    roach = [row for row in data if row[-1] == 'Roach']
    roach = roach[:41]
    pike = [row for row in data if row[-1] == 'Pike']
    pike = pike[:41]
    tree=build_tree(roach+pike)
    prediction=get_max_



    prediction=classify(test_case,tree)
    print(prediction)
